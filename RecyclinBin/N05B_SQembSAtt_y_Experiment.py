...
#====================================================================================================#
# X05D
class SQembWtLr_Model(nn.Module):
    def __init__(self, d_model, d_ff, dropout):
        super().__init__()
        self.weight = weight_norm(nn.Linear(d_model, 1), dim = None)
        self.hidden = weight_norm(nn.Linear(d_model, d_ff), dim = None)
    ...
    def forward(self, input_emb, input_mask):
        ...
        input_mask : [batch_size, src_len]
        '''
        position_weight = nn.Softmax(dim=1)(self.weight(input_emb)*input_mask.masked_fill_(input_mask == 0, -np.inf).unsqueeze(2)).permute(0,2,1) #[batch_size, 1, src_len]
        output_emb = torch.matmul(position_weight, input_emb) #[batch_size, 1, d_model]
        output_emb = torch.flatten(output_emb, start_dim = 1) #[batch_size, d_model + cmpd_dim]
        output = nn.functional.relu(self.hidden(output_emb))

        output = self.feedforward(output)

        #output = self.fc_3(nn.functional.relu(self.fc_2(output)))
        #output = self.fc_3(output)
        return output, position_weight


#====================================================================================================#
# X04B
class SQembAvgD_Model(nn.Module):
    def __init__(self,
                 in_dim  : int   ,
                 hid_1   : int   ,
                 hid_2   : int   ,
        ...
        super(SQembAvgD_Model, self).__init__()
    ...
    def forward(self, input_emb, input_mask):
        ...
        input_emb = input_emb.transpose(1, 2)
...
#====================================================================================================#
# X05-Ensemble
class SAtt_Ensemble(nn.Module):

    def __init__(self, SQembSAtt_Model, SQembAvgP_Model, d_ff):
        super(SAtt_Ensemble, self).__init__()
        self.SQembSAtt_Model = SQembSAtt_Model
        self.SQembAvgP_Model  = SQembAvgP_Model

        self.fc1 = nn.Linear(2, 1)

    def forward(self, input_emb, input_mask):

        out1, _ = self.SQembSAtt_Model(input_emb, input_mask)
        out2, _ = self.SQembAvgP_Model(input_emb, input_mask)
...
'''
# X05D
model = SQembWtLr_Model(  d_model  =  NN_input_dim,
                          d_ff     =  last_hid,
                          dropout  =  0.0
                       )

# Ensemble
SQembSAtt_Model = SQembSAtt_Model(d_model    = NN_input_dim,
                                  d_k        = d_k,
                                  n_heads    = n_heads,
                                  d_v        = d_v,
                                  out_dim    = 1,
...
SQembAvgP_Model = SQembAvgP_Model( in_dim  = NN_input_dim , 
                                   hid_1   = last_hid     , 
                                   hid_2   = last_hid     ,
                                   dropout = dropout      ,
                                   )

model = SAtt_Ensemble( SQembSAtt_Model      ,
                       SQembAvgP_Model      , 
                       d_ff    = last_hid   ,
                       )
...
try:
    optimizer = torch.optim.Adam([
                {'params': model.layers.pos_ffn.weights, "lr": 0.01 , "weight_decay": 0.5, },
            ], lr = learning_rate)
except:
    optimizer = torch.optim.Adam(model.parameters(),lr=learning_rate)
